...
import (
	...
	"encoding/json"
	"errors"
	Store2 "github.com/yametech/global-ipam/pkg/store"
	"net"
...
func New(name string, _IPAMConfig *allocator.IPAMConfig) (*Store, error) {
	etcdClient, err := connectStore(_IPAMConfig.EtcdConfig)
	if err != nil {
		return nil, err
	}
...
func netConfigJson(_IPAMConfig *allocator.IPAMConfig) (string, error) {
	conf, err := json.Marshal(_IPAMConfig.Ranges)
